...
public class RepCacheFailStressT extends AbstractDataCopyTest {
    ...
    public static final String DB_ARCHIVE = ClassLoader.getSystemResource("data/db/rep-cache/fail/rep-cache.zip").getFile();
    ...
    public void repCacheFail() throws Exception {
        ...
        db1.runReadTransaction(db -> {
            ISqlJetCursor c = null;
            final Collection<Object> values = new ArrayList<>();
            final Collection<Collection<Object>> block = new LinkedList<>();
            try {
                ...
                while (!c.eof()) {
                    ...
                    if (rev != currentRev) {
                        db2.runWriteTransaction(db3 -> {
                            for (Collection<Object> row : block) db2.getTable("rep_cache").insert(row.toArray());
                            return null;
                        });
                    ...
                    block.add(new ArrayList<>(values));
                    c.next();
                }
                if (!block.isEmpty()) {
                    db2.runWriteTransaction(db3 -> {
