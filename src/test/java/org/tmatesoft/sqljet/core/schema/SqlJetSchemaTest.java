...
public class SqlJetSchemaTest extends AbstractDataCopyTest {
    ...
    public static final String DB = SqlJetUtility.getSysProp(SCHEMA_TEST + ".DB", ClassLoader.getSystemResource("data/db/testdb.sqlite").getFile());
    ...
    public static final String REP_CACHE_DB = SqlJetUtility.getSysProp(SCHEMA_TEST + ".REP_CACHE_DB",
            ClassLoader.getSystemResource("data/db/rep-cache/rep-cache.db").getFile());
    ...
    public void createTableTest() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            final ISqlJetTableDef createTable = db1.createTable("create table test( id integer primary key, name text )");
            final ISqlJetTable openTable = db1.getTable(createTable.getName());
    ...
    public void createTableTest1() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table test1( id integer primary key, name text )");
            return null;
        });
    ...
    public void createTableTestUnique() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            final ISqlJetTableDef createTable = db1.createTable("create table test( id integer primary key, name text unique )");
            final ISqlJetTable openTable = db1.getTable(createTable.getName());
            logger.info(createTable.toString());
            openTable.insert(null, "test");
            return null;
    ...
    public void createIndexTest() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            ...
            logger.info(createTable.toString());
            openTable.insert(null, "test");
            db1.createIndex("CREATE INDEX test_name_index ON test(name);");
            return null;
        });
    }
    ...
    public void createIndexFailColumn() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            ...
            logger.info(createTable.toString());
            openTable.insert("test");
            db1.createIndex("CREATE INDEX test_name_index ON test(test);");
            return null;
        });
        Assert.assertFalse(false);
    }
    ...
    @Test
    public void createIndexRepCache() throws SqlJetException, IOException {
        final SqlJetDb repCache = SqlJetDb.open(copyFile(new File(REP_CACHE_DB), DELETE_COPY), true);
        repCache.runWriteTransaction(db1 -> {
            db1.createIndex("CREATE INDEX rep_cache_test_index ON " + REP_CACHE_TABLE + "(hash, revision, offset, size, expanded_size);");
    ...
    @Test
    public void createTableRepCache() throws SqlJetException, IOException {
        final SqlJetDb repCache = SqlJetDb.open(copyFile(new File(REP_CACHE_DB), DELETE_COPY), true);
        repCache.runWriteTransaction(db1 -> {
            final ISqlJetTableDef createTable = db1.createTable("create table test( id integer primary key, name text )");
            ...
            logger.info(createTable.toString());
            openTable.insert(null, "test");
            db1.createIndex("CREATE INDEX test_index ON test(name);");
    ...
    @Test
    public void dropTableRepCache() throws SqlJetException, IOException {
        final SqlJetDb repCache = SqlJetDb.open(copyFile(new File(REP_CACHE_DB), DELETE_COPY), true);
        repCache.runWriteTransaction(db1 -> {
            db1.dropTable(REP_CACHE_TABLE);
            return null;
        });
    ...
    public void dropIndex() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            db1.dropIndex("test1_name_index");
            db1.dropIndex("test1_value_index");
            return null;
        });
    ...
    public void dropAll() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            ...
            Set<String> tables = db1.getSchema().getTableNames();
            for (String tableName : tables) {
                ISqlJetTableDef tableDef = db1.getSchema().getTable(tableName);
                Set<ISqlJetIndexDef> tableIndices = db1.getSchema().getIndexes(tableDef.getName());
                for (ISqlJetIndexDef indexDef : tableIndices) {
                    if (!indexDef.isImplicit()) db1.dropIndex(indexDef.getName());
                }
                db1.dropTable(tableName);
            }
            return null;
        });
    }
    ...
    public void createDataBase() throws SqlJetException, IOException {
        ...
        if (DELETE_COPY) createFile.deleteOnExit();

        final SqlJetDb createDb = SqlJetDb.open(createFile, true);
        createDb.runWriteTransaction(db1 -> {
            final ISqlJetTableDef createTable = db1.createTable("create table test( id integer primary key, name text )");
            logger.info(createTable.toString());
            db1.createIndex("CREATE INDEX test_index ON test(name);");
    ...
    @Test
    public void changeEncoding() throws SqlJetException, IOException {
    ...
    @Test(expected = SqlJetException.class)
    public void changeEncodingFail() throws SqlJetException, IOException {
    ...
    @Test
    public void changePageCacheSize() throws SqlJetException, IOException {
    ...
    @Test
    public void changeVacuum() throws SqlJetException, IOException {
    ...
    @Test
    public void changeSchemaVersion() throws SqlJetException, IOException {
    ...
    @Test
    public void changeFileFormatMin() throws SqlJetException, IOException {
    ...
    @Test
    public void changeFileFormatMax() throws SqlJetException, IOException {
    ...
    public void createIndexUniqueFail() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            final ISqlJetTableDef createTable = db1.createTable("create table test( id integer primary key, name text )");
            logger.info(createTable.toString());
            final ISqlJetIndexDef createIndex = db1.createIndex("create unique index index_test_text on test(name)");
        ...
        db.open();
        db.runWriteTransaction(db1 -> {
            final ISqlJetTable openTable = db1.getTable("test");
            openTable.insert(null, "test");
            openTable.insert(null, "test");
            return null;
        });
