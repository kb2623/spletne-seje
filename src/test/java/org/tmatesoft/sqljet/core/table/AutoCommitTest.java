...
public class AutoCommitTest extends AbstractNewDbTest {
    ...
    public void createIndexAutocommit() throws SqlJetException {
        ...
        db.runReadTransaction(db1 -> {
            Assert.assertTrue(!db1.getTable("t").lookup("idx", 1, 1).eof());
            return null;
        });
    }
    ...
    public void dropTableTransaction() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t1(a,b)");
            Assert.assertNotNull(db1.getSchema().getTable("t1"));
            return null;
        });
        db.close();
        db = SqlJetDb.open(file, true);
        db.runWriteTransaction(db1 -> {
            db1.dropTable("t1");
            Assert.assertNull(db1.getSchema().getTable("t1"));
            return null;
        });
    }
    ...
    public void dropIndexTransaction() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            db1.createIndex("create index idx on t(a,b)");
            Assert.assertNotNull(db1.getSchema().getIndex("idx"));
            return null;
        });
        db.close();
        db = SqlJetDb.open(file, true);
        db.runWriteTransaction(db1 -> {
            db1.dropIndex("idx");
            Assert.assertNull(db1.getSchema().getIndex("idx"));
            return null;
        });
    }
