...
public class DbTest extends AbstractNewDbTest {
    ...
    @Test
    public void testOpenRO() throws SqlJetException, IOException {
        Assert.assertTrue(db.isOpen());
    ...
    public void testRowValues() throws SqlJetException {
        ...
        db.runReadTransaction(db1 -> {
            ...
            try {
                ...
                Assert.assertArrayEquals(new Object[] {(long) 1, (long) 555, "a" }, r1);
                c.next();
                Object[] r2 = c.getRowValues();
                Assert.assertArrayEquals(new Object[] {(long) 2, (long) 777, "b" }, r2);
            } finally {
                c.close();
            }
            return null;
            ...
            try {
                {
                    ...
                    Assert.assertEquals((long) 555, r[1]);
                    Assert.assertEquals("a", r[2]);
                }
                c.next();
                {
                    Object[] r = c.getRowValues();
                    Assert.assertEquals((long) 777, r[1]);
                    Assert.assertEquals("b", r[2]);
                }
                c.next();
                {
                    Object[] r = c.getRowValues();
                    Assert.assertEquals((long) 888, r[1]);
                    Assert.assertEquals("c", r[2]);
                }
                c.next();
                {
                    Object[] r = c.getRowValues();
                    Assert.assertEquals((long) 999, r[1]);
                    Assert.assertEquals("d", r[2]);
                }
                c.next();
                {
                    Object[] r = c.getRowValues();
                    Assert.assertEquals((long) 7, r[0]);
                    Assert.assertEquals((long) 111, r[1]);
                    Assert.assertEquals("e", r[2]);
                }
                c.next();
                {
                    Object[] r = c.getRowValues();
                    Assert.assertEquals((long) 8, r[0]);
                    Assert.assertEquals((long) 222, r[1]);
