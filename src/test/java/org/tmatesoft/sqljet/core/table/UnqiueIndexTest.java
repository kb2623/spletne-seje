...
public class UnqiueIndexTest extends AbstractNewDbTest {
    ...
    public void testReplaceCorruptsTable() throws SqlJetException {
        ...
        db.runTransaction(db1 -> {
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B", "unique"));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E/beta", "A/B/E", "unique"));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B/replaced", "unique"));
            return null;
        }, SqlJetTransactionMode.WRITE);
        db.runTransaction(db1 -> {
            final ISqlJetCursor alreadyReplaced = db1.getTable("NODES").lookup(null, new Object[] {1, "A/B/E/beta"});
            try {
                Assert.assertTrue(alreadyReplaced.eof());
            } finally {
                alreadyReplaced.close();
            }
            final ISqlJetCursor present = db1.getTable("NODES").lookup(null, new Object[] {1, "A/B/E"});
            try {
                Assert.assertEquals("A/B/replaced", present.getString("parent_relpath"));
            } finally {
                present.close();
            }
            final ISqlJetCursor all = db1.getTable("NODES").open();
            try {
    ...
    public void testReplaceReplacesWrongRow() throws SqlJetException {
        createTables();
        db.runTransaction(db1 -> {
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B", "not_unique"));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E/beta", "A/B/E", "unique"));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B/replaced", "unique"));
            return null;
        }, SqlJetTransactionMode.WRITE);
    ...
    public void testNullsAreDistinctInUniqueColumn() throws SqlJetException {
        createTables();
        db.runTransaction(db1 -> {
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B", null));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E/beta", "A/B/E", null));
            db1.getTable("NODES").insertByFieldNamesOr(SqlJetConflictAction.REPLACE, getRowForPath("A/B/E", "A/B/replaced", null));
            return null;
        }, SqlJetTransactionMode.WRITE);
        db.runTransaction(db1 -> {
            final ISqlJetCursor all = db1.getTable("NODES").open();
            ...
            final ISqlJetCursor twoRows = db1.getTable("NODES").lookup(null, new Object[] {1, "A/B/E"});
            try {
                Assert.assertEquals(1, twoRows.getRowCount());
            } finally {
                twoRows.close();
            }
            final ISqlJetCursor oneRow = db1.getTable("NODES").lookup(null, new Object[] {1, "A/B/E/beta"});
            try {
