...
public class IndexScopeTest extends AbstractNewDbTest {
    ...
    public void setUp() throws Exception {
        ...
        db.runWriteTransaction(db1 -> {
            ...
            db1.createTable("create table t(a integer primary key, b integer)");
            db1.createIndex("create index b on t(b)");
            db1.createIndex("create index ab on t(a,b)");

            db1.createTable("create table t1(a integer primary key, b integer)");
            db1.createIndex("create index b1 on t1(b)");

            db1.createTable("create table t2(a integer, b integer, primary key(a,b))");
            db1.createTable("create table t3(a integer primary key)");

            db1.createTable("create table t4(a text not null primary key, b integer not null)");
            db1.createIndex("create index b4 on t4(b)");

            db1.createTable("create table t5(a integer)");

            table = db1.getTable("t");

            for (int i = 10; i > 0; i--) {
                table.insert(null, i);
            }

            table1 = db1.getTable("t1");
            table1.insert(null, 3);
            table1.insert(null, 5);
            table1.insert(null, 7);
            table1.insert(null, 9);

            table2 = db1.getTable("t2");

            for (int i = 10; i > 0; i--) {
                table2.insert(i, i);
            }

            table3 = db1.getTable("t3");
            table3.insert(3);
            table3.insert(5);
            table3.insert(7);
            table3.insert(9);

            table4 = db1.getTable("t4");
            table4.insert("s", 10);
            table4.insert("q", 4);
            table4.insert("l", 3);
            table4.insert("j", 2);
    ...
    public void testNoAssertionOnGetRowId() throws SqlJetException {
        db.runReadTransaction(db1 -> {
            for(String tableName : db1.getSchema().getTableNames()) {
                ISqlJetTable table6 = db1.getTable(tableName);
                Set<ISqlJetIndexDef> indices = db1.getSchema().getIndexes(tableName);
                if (indices != null) {
                    for (ISqlJetIndexDef indexDef : indices) {
                        SqlJetScope scope = new SqlJetScope((SqlJetScopeBound) null, null);
                        ISqlJetCursor cursor = table6.scope(indexDef.getName(), scope);
