...
public class SqlJetIndexTest extends TestCase {
    ...
    private void createTableT() throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a text, b text, c int, d int)");
            ISqlJetTable t = db1.getTable("t");
            t.insert("n", "y", 10, 20);
            t.insert("x", "z", 11, 12);
            t.insert("a", "b", 10, 13);
            t.insert("c", "b", 10, 23);
    ...
    public void testSingleColumnIndex() throws Exception {
        createTableT();
        db.runWriteTransaction(db1 -> {
            db1.createIndex("create index tb on t (b)");
            return null;
        });
    ...
    public void testMultiColumnIndex() throws Exception {
        createTableT();
        db.runWriteTransaction(db1 -> {
            db1.createIndex("create index tbc on t (b,c)");
            return null;
        });
        db.runReadTransaction(db1 -> {
            ISqlJetTable t = db1.getTable("t");
    ...
    public void testAutomaticIntConversion() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a int)");
            db1.createIndex("create index ta on t (a)");
            ISqlJetTable t = db1.getTable("t");
            t.insert(10L);
            t.insert(20L);
            return null;
    ...
    public void testAutomaticFloatConversion() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a real)");
            db1.createIndex("create index ta on t (a)");
            ISqlJetTable t = db1.getTable("t");
            t.insert(0.1D);
            t.insert(0.2D);
    ...
    public void testReadUsingColumnPK() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a int primary key, b text)");
            ISqlJetTable t = db1.getTable("t");
            t.insert(1, "zzz");
            t.insert(2, "www");
            return null;
    ...
    public void testReadUsingColumnPKAutoinc() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a integer primary key autoincrement, b text)");
            ISqlJetTable t = db1.getTable("t");
    ...
    public void testReadUsingSecondColumnPK() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a text, b int primary key)");
            ISqlJetTable t = db1.getTable("t");
            t.insert("zzz", 1);
            t.insert("www", 2);
            return null;
    ...
    public void testReadUsingSingleColumnTablePK() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a text, b text, primary key (a))");
            ISqlJetTable t = db1.getTable("t");
            t.insert("set", "in");
            t.insert("get", "out");
            t.insert("bet", "down");
    ...
    public void testReadUsingMultiColumnTablePK() throws Exception {
        db.runWriteTransaction(db1 -> {
            db1.createTable("create table t (a text, b text, primary key (a,b))");
            ISqlJetTable t = db1.getTable("t");
            t.insert("get", "in");
            t.insert("get", "out");
            t.insert("get", "down");
