...
public class SqlJetBtreeTest extends SqlJetAbstractLoggedTest {
    ...
    private File testDataBase = new File(ClassLoader.getSystemResource("data/db/testdb.sqlite").getFile());
    private File testWriteNativeFile = new File(ClassLoader.getSystemResource("data/db/write.native").getFile());
    private File testDeleteNativeFile = new File(ClassLoader.getSystemResource("data/db/delete.native").getFile());
    private File testUpdateNativeFile = new File(ClassLoader.getSystemResource("data/db/update.native").getFile());
    ...
    public void tearDown() throws Exception {
        if (testTempFile != null) SqlJetFileUtil.deleteFile(testTempFile);
    }
    ...
    private boolean compareFiles(File f1, File f2) throws Exception {
        try (RandomAccessFile i1 = new RandomAccessFile(f1, "r")) {
            try (RandomAccessFile i2 = new RandomAccessFile(f2, "r")) {
                ...
                if (m1.hasRemaining() || m2.hasRemaining()) return false;
            }
        }
        return true;
    }
    ...
    public void testDelete() throws Exception {
        ...
        try {
            ...
            try {
                ...
                btree.beginTrans(SqlJetTransactionMode.WRITE);
                for (int x = 1; x <= 3; x++) {
                    final int table = btree.createTable(SqlJetUtility.of(SqlJetBtreeTableCreateFlags.INTKEY, SqlJetBtreeTableCreateFlags.LEAFDATA));
                    final ISqlJetBtreeCursor c = btree.getCursor(table, true, null);
                    c.enterCursor();
                    try {
                        for (int y = 1; y <= 3; y++) c.insert(null, y, pData, pData.remaining(), 0, false);
                        c.closeCursor();
                    } finally {
                        c.leaveCursor();
                    }
                    ...
                    try {
                        ...
                        if (c.first()) {
                            ...
                            do {
                                ...
                                ISqlJetMemoryPointer data = SqlJetUtility.allocatePtr(dataSize);
                                c.data(0, dataSize, data);
                                final String str = SqlJetUtility.toString(data, SqlJetEncoding.UTF8).replaceAll("[^\\p{Print}]", "?");
                                logger.info("record " + key + " : \"" + str + "\"");
                            } while (!c.next());
                        }
                    } finally {
