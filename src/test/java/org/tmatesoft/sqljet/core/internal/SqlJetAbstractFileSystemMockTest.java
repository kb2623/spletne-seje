...
public abstract class SqlJetAbstractFileSystemMockTest extends SqlJetAbstractMockTest {
     ...
     */
    public static final Set<SqlJetFileOpenPermission> PERM_TEMPORARY = SqlJetUtility.of(SqlJetFileOpenPermission.CREATE, SqlJetFileOpenPermission.READWRITE, SqlJetFileOpenPermission.DELETEONCLOSE);
    /**
    ...
    protected void setUpInstances() throws Exception {
        ...
        if(null==fileSystemsManager) fileSystemsManager = EasyMock.createNiceMock(ISqlJetFileSystemsManager.class);
        ...
        EasyMock.expect(fileSystem.open(EasyMock.isA(File.class), EasyMock.isNull(), EasyMock.isNull())).andStubThrow(cantOpen);

        EasyMock.expect(fileSystem.open(EasyMock.isA(File.class), EasyMock.isA(SqlJetFileType.class), EasyMock.isNull())).andStubThrow(cantOpen);

        EasyMock.expect(fileSystem.open(EasyMock.isA(File.class), EasyMock.isNull(), EasyMock.isA(Set.class))).andStubThrow(cantOpen);
        ...
        EasyMock.expect(fileSystem.open(
                EasyMock.or(EasyMock.isNull(),EasyMock.isA(File.class)),
                EasyMock.or(EasyMock.isNull(),EasyMock.isA(SqlJetFileType.class)),
        ...
        EasyMock.expect(fileSystem.delete(
                        EasyMock.isNull(), EasyMock.anyBoolean() )
        ...
        EasyMock.expect(fileSystem.access(
                        EasyMock.anyObject(), EasyMock.isNull())
        ).andStubThrow(cantAccess);

        EasyMock.expect(fileSystem.access(
                        EasyMock.eq(path), EasyMock.anyObject())
        ).andStubReturn(true);

        EasyMock.expect(fileSystem.access(
                        EasyMock.eq(pathNew), EasyMock.anyObject())
        ).andStubReturn(false);
    ...
    protected void cleanUpInstances() throws Exception {
        ...
        if (fileSystemsManager != null && fileSystem != null) fileSystemsManager.unregister(fileSystem);
