...
public class Svn17WCDao {
    ...
    private static final File WC = new File(ClassLoader.getSystemResource("data/db/svn1.7/wc.db").getFile());
    ...
    public Map<String, Object> queryBaseNodeWithLock(final long wc_id, final String local_relpath,
        ...
        return (Map<String, Object>) db.runReadTransaction(db1 -> {
            final ISqlJetCursor base_node = db1.getTable(BASE_NODE).lookup(null, wc_id, local_relpath);
            if (base_node.eof()) {
                ...
                if (withLock) {
                    final ISqlJetCursor lock = db1.getTable(LOCK).lookup(null,
