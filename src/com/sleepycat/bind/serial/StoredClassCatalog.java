...
public class StoredClassCatalog implements ClassCatalog {
    ...
    public StoredClassCatalog(Database database) throws DatabaseException, IllegalArgumentException {
        ...
        writeLockMode = (DbCompat.getInitializeLocking(envConfig) || envConfig.getTransactional()) ? LockMode.RMW : LockMode.DEFAULT;
        cdbMode = DbCompat.getInitializeCDB(envConfig);
        txnMode = dbConfig.getTransactional();

        if (!DbCompat.isTypeBtree(dbConfig)) throw new IllegalArgumentException("The class catalog must be a BTREE database.");
        ...
        if (dbConfig.getReadOnly()) {
             ...
             */
            OperationStatus status = db.get(null, key, data, null);
            if (status == OperationStatus.NOTFOUND) db.putNoOverwrite(null, key, data);
        }
    }
    ...
    private ClassInfo putClassInfo(ClassInfo classInfo,
        ...
        try {
            ...
            if (status != OperationStatus.SUCCESS) throw DbCompat.unexpectedState("Class ID not initialized");
    ...
    private static byte[] getBytes(DatabaseEntry dbt) {
        ...
        if (dbt.getOffset() == 0 && b.length == dbt.getSize()) return b;
        int len = dbt.getSize();
        if (len == 0) {
