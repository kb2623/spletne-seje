...
public class LogManager {
    ...
    public void log(LogItem item, LogContext context)
        ...
        try {
            ...
            while (lqe != null) {
                LogItem lqeItem = new LogItem();
                lqeItem.entry = lqe.entry;
                lqeItem.provisional = Provisional.NO;
                lqeItem.repContext = lqe.repContext;

                LogContext lqeContext = new LogContext();

                logItem(lqeItem, lqeContext);
    ...
    private void logItem(LogItem item, LogContext context)
        ...
        final LogWriteInfo lwi = serialLog(
            item, context, context.forceNewLogFile, flushRequired, tracker);
    ...
    */
        ...
        synchronized (logWriteMutex) {
            ...
            try {
                return serialLogWork(
                    item, context, forceNewLogFile, flushRequired, tracker);
            } catch (EnvironmentFailureException e) {
    ...
    private LogWriteInfo serialLogWork(
        ...
        try {
             ...
             */
            if (item.entry.isImmediatelyObsolete(context.nodeDb)) {
