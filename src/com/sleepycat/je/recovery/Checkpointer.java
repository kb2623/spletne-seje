...
public class Checkpointer extends DaemonThread implements EnvConfigObserver {
    ...
    public StatGroup loadStats(StatsConfig config) {
        ...
        new LSNStat(stats, CKPT_LAST_CKPT_INTERVAL, lastCheckpointInterval);
    ...
    private static void flushIN(EnvironmentImpl envImpl,
        ...
        if (!targetWasRoot) {
            ...
            SearchResult result = tree.getParentINForChildIN(
                targetRef.nodeId, targetRef.treeKey, -1, /*targetLevel*/
            ...
            if (result.parent != null) {
                ...
                try {
                    if (result.exactParentFound) {
                         ...
                         */
                        IN renewedTarget =
                            (IN) parent.getTarget(result.index);
                        ...
                        if (renewedTarget == null) {
                            ...
                            if (logSiblingsWithParentLatchHeld) {
                                ...
                                logSiblings(
                                    ...
                                    allowDeltas, provisional, fstats);
                                mustLogParent = true;
                            }
                        }
                    } else {
                    ...
                    if (mustLogParent) {
                         ...
                         * even if not dirty, to cause changes to propagate
                ...
                if (siblingsToLog != null) {
                    ...
                    for (long childId : siblingsToLog.keySet()) {
                        ...
                        if (childRef != null) {
                            flushIN(
                                envImpl, db, logManager, childRef, dirtyMap,
                                currentLevel, maxFlushLevel, allowDeltas,
                                highPriority, fstats,
     ...
     */
    private static void logSiblings(
        ...
        for (int index : indicesToLog) {
            ...
            try {
                ...
                parent.updateEntry(
                    item.parentIndex, item.newLsn,
                    VLSN.NULL_VLSN.getSequence(), 0/*lastLoggedSize*/);
