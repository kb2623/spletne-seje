...
public class LN extends Node implements VersionedWriteLoggable {
     ...
     */
    @Override
    void accountForDeferredWriteSubtreeRemoval(
        INList inList,
        IN subtreeParent) {
        /* Don't remove, LNs not on this list. */
    }
    ...
    public LogResult optionalLog(
        ...
        if (dbImpl.isDeferredWriteMode() && currEmbeddedLN == newEmbeddedLN) {
            return new LogResult(
                assignTransientLsn(envImpl, dbImpl, currLsn, locker), -1);
        } else {
            return logInternal(
