...
public class IN extends Node implements Comparable<IN>, LatchContext {
     ...
     * IN.commonPostFetchInit(), via IN.setLastLoggedLsn()). If L is a BIN
     ...
     * full-BIN logrec (this is done in IN.commonPostFetchInit(), via
    ...
    public final void setGeneration(CacheMode cacheMode) {
        ...
        case MAKE_COLD:
            ...
            break;
        default:
            throw unexpectedState("unknown cacheMode: " + cacheMode);
        }
    }
    ...
    private final void setFetchedCold(boolean val) {
        if (val) {
    ...
    final Node fetchLN(int idx, CacheMode cacheMode, boolean dupConvert) {
        ...
        if (cacheMode != CacheMode.UNCHANGED && targetNode.isLN()) {
            ((LN) targetNode).setFetchedCold(false);
        }
    ...
    public final void postFetchInit(DatabaseImpl db, long fetchedLsn) {
        ...
        commonPostFetchInit(db, fetchedLsn);
        ...
        /*
         * This flag is initially true for a fetched BIN, and will be set to
         * false if the BIN is latched with any CacheMode other than UNCHANGED.
         */
        if (isBIN()) {
            setFetchedCold(true);
        }
     ...
     */
    private void commonPostFetchInit(DatabaseImpl db, long fetchedLsn) {
        setDatabase(db);
    ...
    public final void insertRecord(
        ...
        if (isEmbeddedLN(idx)) {
            ...
            ((BIN)this).setCachedVLSN(idx, newLN.getVLSNSequence());
    ...
    public final void recoverRecord(
        ...
        if (isEmbeddedLN(idx)) {
            clearLastLoggedSize(idx);
            ((BIN)this).setCachedVLSN(idx, vlsn);
        } else {
     ...
     *
     * It is called from IN.commonPostFetchInit(). If the logrec we have just
     * read was a BINDelta, this.lastFullVersion has already been set (in
     ...
     * and it will be set here. In this case, this.lastDeltaVarsion will
     * remain NULL.
     */
    private final void setLastLoggedLsn(long lsn) {
        if (isBIN()) {
            if (getLastFullLsn() == DbLsn.NULL_LSN) {
                setLastFullLsn(lsn);
            } else {
            ...
            if (child != null) {
                ...
                try {
                    if (child.getDirty()) {
                        ...
                        updateEntry(
                            i, childLsn, VLSN.NULL_VLSN.getSequence(),
                            0/*lastLoggedSize*/);
                    }
                } finally {
                    child.releaseLatch();
    ...
    public final void writeToLog(ByteBuffer logBuffer, boolean deltasOnly) {
        ...
        */
        ...
        for (int i = 0; i < nEntries; i++) {
            ...
            LogUtils.writeByteArray(logBuffer, entryKeys.get(i)); // key
    ...
    public final void dumpLog(StringBuilder sb, boolean verbose) {
        ...
        if (verbose) {
            for (int i = 0; i < nEntries; i++) {
                ...
                sb.append("\" no data=\"").append(isNoDataLN(i));
                sb.append("\" logSize=\"");
                if (isBIN()) {
                ...
                sb.append("\">");
                sb.append(Key.dumpString(entryKeys.get(i), 0));
                if (keyPrefix != null || isEmbeddedLN(i)) {
                    sb.append(Key.dumpString(getKey(i), "actual_key", 0));
                }
