...
class FileProcessor extends DaemonThread {
        ...
        try {
            ...
            if (!obsolete && !lockDenied) {
                ...
                if (db.getId().equals(DbTree.ID_DB_ID)) {
                    ...
                    final LN.LogResult logResult = targetLn.log(
                    ...
                    bin.updateEntry(
                        index, logResult.newLsn, targetLn.getVLSNSequence(),
                        logResult.newSize);
                    ...
                    CursorImpl.lockAfterLsnChange(
                        db, logLsn, logResult.newLsn, locker /*excludeLocker*/);
                    nLNsMigratedThisRun++;
                }

                /*
                 * Update the generation so that the BIN is not evicted
                 * immediately.  This allows the cleaner to migrate as many
                 * entries as possible before eviction of the BIN.
                 */
    ...
    private IN findINInTree(
        ...
        try {
             ...
             */
            final BIN bin = (BIN)parent.fetchIN(result.index);
