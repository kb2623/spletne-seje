...
public final class PasswordCredential implements java.io.Serializable {
    ...
    public boolean equals(Object other) {
        if (!(other instanceof PasswordCredential)) return false;
        PasswordCredential pc = (PasswordCredential)other;
        if (!(userName.equals(pc.userName))) return false;
        if (password.length != pc.password.length) return false;
        for (int i = 0; i < password.length;i++) {
            if (password[i] != pc.password[i]) return false;
        }
        return true;
    }
    ...
    int hashCode() {
        String s = userName;
        int passwordHash = 0;
        for (char passChar : password) passwordHash += passChar;
