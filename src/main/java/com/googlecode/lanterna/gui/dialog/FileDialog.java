...
public class FileDialog extends Window {
    ...
    private FileDialog(File directory, String title, Kind kind) {
        ...
        Button okButton = new Button(kind.name(), () -> {
            selectedFile = new File(currentDirectory, fileText.getText());
            close();
        });
        Button cancelButton = new Button("Cancel", FileDialog.this::close);
    ...
    private void reloadViews(File directory) {
        ...
        Arrays.sort(entries, (o1, o2) -> o1.getName().toLowerCase().compareTo(o2.getName().toLowerCase()));
        dirView.addAction("..", () -> reloadViews(currentDirectory.getParentFile()));
        for(final File entry: entries) {
            if(entry.isDirectory()) {
                dirView.addAction(entry.getName(), () -> reloadViews(entry));
            } else {
                fileView.addAction(entry.getName(), () -> {
