		...
		if (fieldsTypes.size() < 1) {
			...
			if (fields.size() < fieldsTypes.size()) {
				/** Inicializacije UserSessino razreda */{
					builder.setLength(0);
					builder.append("private " + List.class.getName() + " session;");
					CtField field = CtField.make(builder.toString(), aClass);
					ConstPool constPool = field.getFieldInfo().getConstPool();
					AnnotationsAttribute attr = new AnnotationsAttribute(constPool, AnnotationsAttribute.visibleTag);
					/** Dodajanje anoracije ManyToMany */{
						Annotation anno = new Annotation(OneToMany.class.getName(), constPool);
						{
							...
							ArrayMemberValue array = new ArrayMemberValue(constPool);
							array.setValue(new MemberValue[]{value});
							anno.addMemberValue("cascade", array);
						}
						{
				...
				/** getter za Session */{
					builder.setLength(0);
					builder.append("public " + List.class.getName() + " getSession() {" + "return this.session;" + "}");
					CtMethod method = CtMethod.make(builder.toString(), aClass);
					aClass.addMethod(method);
				}
				/** setter za Session */{
					builder.setLength(0);
					builder.append("public void setSession(" + List.class.getName() + " session) {" + "this.session = session;" + "}");
					CtMethod method = CtMethod.make(builder.toString(), aClass);
					aClass.addMethod(method);
				}
				/** getLocalTime() */{
