...
public class SqlJetBtreeRecord implements ISqlJetBtreeRecord {
    ...
    public ISqlJetMemoryPointer getRawRecord() {
        ...
        for (ISqlJetVdbeMem value : fields) {
            ...
            int len;
            if (pRec.flags.contains(SqlJetVdbeMemFlags.Zero) && pRec.n > 0) pRec.expandBlob();
        ...
        if (nVarint < SqlJetUtility.varintLen(nHdr)) nHdr++;
        ...
        for (ISqlJetVdbeMem value : fields) {
            SqlJetVdbeMem pRec = (SqlJetVdbeMem) value;
            /* serial data */
            i += SqlJetVdbeSerialType.serialPut(SqlJetUtility.pointer(zNewRecord, i), nByte - i, pRec, file_format);
        }
        assert (i == nByte);
    ...
    public void release() {
        fields.forEach(org.tmatesoft.sqljet.core.internal.ISqlJetVdbeMem::release);
    }
}
