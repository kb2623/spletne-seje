...
public class SqlJetFileSystemsManager implements ISqlJetFileSystemsManager {
    ...
    public ISqlJetFileSystem find(final String name) {
        if (null != name) return fileSystems.get(name);
        else
            synchronized (lock) {
    ...
    private void checkFS(final ISqlJetFileSystem fs) throws SqlJetException {
        if(null==fs) throw new SqlJetException(SqlJetErrorCode.BAD_PARAMETER, "Prameter 'fs' must be not null");
        if(null==fs.getName()) throw new SqlJetException(SqlJetErrorCode.BAD_PARAMETER, "fs.getName() must return not null value");
    }
