...
public class SqlJetFileUtil {
    ...
    public static boolean deleteFile(File file, boolean sync) {
        ...
        if (OS.isWindows()) sync = true;
        if (!sync || file.isDirectory() || !file.exists()) return file.delete();
        long sleep = 1;
    ...
    public static class SqlJetOsType {
        ...
        public SqlJetOsType() {
            ...
            boolean windows = osName != null && osNameLC.contains("windows");
            if (!windows && osName != null) {
                windows = osNameLC.contains("os/2");
                isOS2 = windows;
            } else {
                isOS2 = false;
            }
            ...
            isWindows = windows;
            isOSX = osName != null && (osNameLC.contains("mac") || osNameLC.contains("darwin"));
            isLinux = osName != null && (osNameLC.contains("linux") || osNameLC.contains("hp-ux"));
            isBSD = !isLinux && osName != null && osNameLC.contains("bsd");
            isSolaris = !isLinux && !isBSD && osName != null && (osNameLC.contains("solaris") || osNameLC.contains("sunos"));
            isOpenVMS = !isOSX && osName != null && osNameLC.contains("openvms");
