...
public class SqlJetBtree implements ISqlJetBtree {
    ...
    static void TRACE(String format, Object... args) {
        if (SQLJET_LOG_BTREE) SqlJetUtility.log(btreeLogger, format, args);
    }
    ...
    public boolean invokeBusyHandler(int number) {
        ...
        return busyHandler != null && busyHandler.call(number);
    }
    ...
    public void open(File filename, ISqlJetDbHandle db, Set<SqlJetBtreeFlags> flags, final SqlJetFileType type, final Set<SqlJetFileOpenPermission> permissions) throws SqlJetException {
        ...
        try {
            if (this.pBt == null) {
                ...
                this.pBt = pBt;
                pBt.pPager.setReiniter(SqlJetBtree.this::pageReinit);
    ...
    public void setPageSize(int pageSize, int reserve) throws SqlJetException {
        ...
        enter();
        try {
            if (pBt.pageSizeFixed) throw new SqlJetException(SqlJetErrorCode.READONLY);
            if (reserve < 0) reserve = pBt.pageSize - pBt.usableSize;
    ...
    public SqlJetAutoVacuumMode getAutoVacuum() {
        enter();
        try {
            return !pBt.autoVacuum ? SqlJetAutoVacuumMode.NONE : !pBt.incrVacuum ? SqlJetAutoVacuumMode.FULL : SqlJetAutoVacuumMode.INCR;
        } finally {
            leave();
        }
    }
    ...
    private void lockBtree() throws SqlJetException {
        ...
        try {
            ...
            if (nPage > 0) {
                ...
                if (SqlJetUtility.memcmp(page1, zMagicHeader, 16) != 0) throw new SqlJetException(rc);
                if (SqlJetUtility.getUnsignedByte(page1, 18) > 1) pBt.readOnly = true;
                if (SqlJetUtility.getUnsignedByte(page1, 19) > 1) throw new SqlJetException(rc);
                 ...
                 * fixed.
                 */
                if (SqlJetUtility.memcmp(page1, 21, PAGE1_21, 0, 3) != 0) throw new SqlJetException(rc);

                pageSize = SqlJetUtility.get2byte(page1, 16);
                if (((pageSize - 1) & pageSize) != 0 || pageSize < ISqlJetLimits.SQLJET_MIN_PAGE_SIZE || (ISqlJetLimits.SQLJET_MAX_PAGE_SIZE < 32768)) throw new SqlJetException(rc);
                assert ((pageSize & 7) == 0);
    ...
    public void beginTrans(SqlJetTransactionMode mode) throws SqlJetException {
        ...
        try {
             ...
             * is a no-op.
             */
            if (inTrans == TransMode.WRITE || (inTrans == TransMode.READ && mode == SqlJetTransactionMode.READ_ONLY)) return;

            /* Write transactions are not possible on a read-only database */
            if (pBt.readOnly && mode != SqlJetTransactionMode.READ_ONLY) throw new SqlJetException(SqlJetErrorCode.READONLY);
             ...
             */
            if (pBt.inTransaction == TransMode.WRITE && mode != SqlJetTransactionMode.READ_ONLY) throw new SqlJetException(SqlJetErrorCode.BUSY);

            if (mode == SqlJetTransactionMode.EXCLUSIVE) {
                for (SqlJetBtreeLock aPLock : pBt.pLock) {
                    if (aPLock.pBtree != this) throw new SqlJetException(SqlJetErrorCode.BUSY);
                }
            }
    ...
    public void commitPhaseOne(String master) throws SqlJetException {
        if (this.inTrans == TransMode.WRITE) {
            ...
            try {
                pBt.db = this.db;
                if (pBt.autoVacuum) pBt.autoVacuumCommit();
    ...
    private void unlockAllTables() {
        ...
        while (ppIter.hasNext()) {
            ...
            if (pLock.pBtree == this) ppIter.remove();
        }

        if (pBt.pExclusive == this) pBt.pExclusive = null;
    ...
    public void commitStmt() throws SqlJetException {
        ...
        try {
            ...
            if (pBt.inStmt) {
                try {
                    pBt.pPager.savepoint(SqlJetSavepointOperation.RELEASE, this.db.getSavepointNum());
                } finally {
                    pBt.inStmt = false;
                }
            }
    ...
    public void rollbackStmt() throws SqlJetException {
        ...
        try {
            ...
            if (pBt.inStmt)
                try {
                    pBt.pPager.savepoint(SqlJetSavepointOperation.ROLLBACK, this.db.getSavepointNum());
                    pBt.pPager.savepoint(SqlJetSavepointOperation.RELEASE, this.db.getSavepointNum());
                } finally {
                    pBt.inStmt = false;
                }
    ...
    protected void pageReinit(ISqlJetPage page) throws SqlJetException {
        ...
        if (pPage != null && pPage.isInit) {
            ...
            pPage.isInit = false;
            if (page.getRefCount() > 0) pPage.initPage();
        }
    }
    ...
    private boolean queryTableLock(int iTab, SqlJetBtreeLockMode eLock) {
         ...
         * not change.
         */
        if (!db.getFlags().contains(SqlJetDbFlags.ReadUncommitted) || eLock == SqlJetBtreeLockMode.WRITE || iTab == ISqlJetDbHandle.MASTER_ROOT) {
            for (SqlJetBtreeLock pIter : pBt.pLock) {
                if (pIter.pBtree != this && pIter.iTable == iTab && (pIter.eLock != eLock || eLock != SqlJetBtreeLockMode.READ)) return false;
            }
        }
        return true;
    }
    ...
    public void lockTable(int table, boolean isWriteLock) {
        if (sharable) {
            ...
            try {
                ...
                if (queryTableLock(table, lockType)) lockTable(table, lockType);
            } finally {
                leave();
            }
        }
    ...
    private void lockTable(int iTable, SqlJetBtreeLockMode eLock) {
         ...
         */
        if (db.getFlags().contains(SqlJetDbFlags.ReadUncommitted) && (eLock == SqlJetBtreeLockMode.READ) && iTable != ISqlJetDbHandle.MASTER_ROOT) return;
    ...
    private void doCopyFile(SqlJetBtree pFrom) throws SqlJetException {
        ...
        if (pBtTo.pCursor != null) throw new SqlJetException(SqlJetErrorCode.BUSY);
    ...
    private int doDropTable(int iTable) throws SqlJetException {
        ...
        if (iTable > 1) {
            if (pBt.autoVacuum) {
                 ...
                 */
                maxRootPgno--;
                if (maxRootPgno == pBt.PENDING_BYTE_PAGE()) maxRootPgno--;
                if (maxRootPgno == pBt.PTRMAP_PAGENO(maxRootPgno)) maxRootPgno--;
