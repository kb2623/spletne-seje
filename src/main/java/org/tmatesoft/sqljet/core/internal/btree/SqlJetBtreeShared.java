...
public class SqlJetBtreeShared {
    ...
    public void invalidateAllOverflowCache() {
        assert (mutex.held());
        for (SqlJetBtreeCursor p = pCursor; p != null; p = p.pNext) p.aOverflow = null;
    }
    ...
    public void ptrmapPut(int key, short eType, int parent) throws SqlJetException {
        ...
        assert (autoVacuum);
        if (key == 0) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
    ...
    public void ptrmapGet(int key, short[] pEType, int[] pPgno) throws SqlJetException {
        ...
        if (pPgno != null && pPgno.length > 0) pPgno[0] = SqlJetUtility.get4byte(pPtrmap, offset + 1);

        pDbPage.unref();

        if (pEType[0] < 1 || pEType[0] > 5) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
    }
        ...
        try {
            if (n > 0) {
                ...
                if (exact && nearby <= getPageCount()) {
                    ...
                    if (eType[0] == PTRMAP_FREEPAGE) searchList = true;
                    pPgno[0] = nearby;
                }
    ...
    public void clearDatabasePage(int pgno, boolean freePageFlag, int[] pnChange) throws SqlJetException {
        ...
        assert (mutex.held());
        if (pgno > pPager.getPageCount()) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
        ...
        try {
            ...
            for (i = 0; i < pPage.nCell; i++) {
                ...
                if (!pPage.leaf) clearDatabasePage(SqlJetUtility.get4byte(pCell), true, pnChange);
    ...
    SqlJetMemPage getAndInitPage(int pgno) throws SqlJetException {
        ...
        try {
            ...
            if (!pPage.isInit) pPage.initPage();
        } catch (SqlJetException e) {
            SqlJetMemPage.releasePage(pPage);
            throw e;
        }
    ...
    public void getOverflowPage(int ovfl, SqlJetMemPage[] ppPage, int[] pPgnoNext) throws SqlJetException {
        ...
        if (autoVacuum) {
            ...
            if (iGuess <= pPager.getPageCount()) {
                ...
                if (eType[0] == PTRMAP_OVERFLOW2 && pgno[0] == ovfl) next = iGuess;
            }
        }
        ...
        if (next == 0 || (ppPage != null && ppPage.length != 0)) {
            ...
            try {
                ...
                if (next == 0 && pPage != null) next = SqlJetUtility.get4byte(pPage.aData, 0);
    ...
    public void allocateTempSpace() {
        if (pTmpSpace == null) pTmpSpace = SqlJetUtility.allocatePtr(pageSize);
    }
