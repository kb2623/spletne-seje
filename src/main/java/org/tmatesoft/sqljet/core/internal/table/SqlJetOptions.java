...
public class SqlJetOptions implements ISqlJetOptions {
    ...
    public String toString() {
        return ("ENCODING: " + encoding + "\n") + "SCHEMA VERSION: " + schemaCookie + "\n" + "USER VERSION: " + userCookie + "\n" + "FILE FORMAT: " + fileFormat + "\n" + "AUTOVACUUM: " + autovacuum + "\n" + "CACHE SIZE: " + pageCacheSize;
    }

    private SqlJetEncoding readEncoding() throws SqlJetException {
        switch (btree.getMeta(ENCODING)) {
        case 0:
            if (readSchemaCookie() != 0) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
        case 1:
    ...
    public void setSchemaVersion(int version) throws SqlJetException {
        ...
        try {
            if (!btree.isInTrans()) throw new SqlJetException("It can be performed only in active transaction");
    ...
    public boolean verifySchemaVersion(boolean throwIfStale) throws SqlJetException {
        ...
        try {
            ...
            if (stale && throwIfStale) throw new SqlJetException(SqlJetErrorCode.SCHEMA);
            return !stale;
        } finally {
            dbHandle.getMutex().leave();
        }
    ...
    private void initMeta() throws SqlJetException {
        ...
        try{
            if(!inTrans || transMode!=SqlJetTransactionMode.EXCLUSIVE) btree.beginTrans(SqlJetTransactionMode.EXCLUSIVE);
            try {
                schemaCookie = 1;
                writeSchemaCookie(schemaCookie);
    ...
    private void checkPageCacheSize() throws SqlJetException {
        if (pageCacheSize < SqlJetPageCache.PAGE_CACHE_SIZE_MINIMUM) pageCacheSize = SqlJetPageCache.PAGE_CACHE_SIZE_DEFAULT;
    }
    ...
    private void checkSchema() throws SqlJetException {
        if (readSchemaCookie() != 1) throw new SqlJetException(SqlJetErrorCode.MISUSE);
    }
    ...
    public void setFileFormat(int fileFormat) throws SqlJetException {
        ...
        try {
            checkSchema();
            if (btree.isInTrans()) throw new SqlJetException("It can't be performed in active transaction");
