...
public class SqlJetPager implements ISqlJetPager, ISqlJetLimits, ISqlJetPageCallback {
    ...
    static boolean subjRequiresPage(SqlJetPage pPg) {
        ...
        for (i = 0; i < pPager.nSavepoint; i++) {
            ...
            if (p.nOrig >= pgno && false == SqlJetUtility.bitSetTest(p.pInSavepoint, pgno)) return true;
        }
        return false;
    }
    ...
    public ISqlJetPage acquirePage(final int pageNumber, final boolean read) throws SqlJetException {
        ...
        if (null == page) throw new SqlJetException(SqlJetErrorCode.INTERNAL, "Page cache is overflow");
    ...
    private void endTransaction(boolean hasMaster) throws SqlJetException {
        ...
        if (journalOpen) {
            if (journalMode == SqlJetPagerJournalMode.MEMORY) {
                ...
                try {
                    jfd.truncate(0);
                } catch (SqlJetIOException ignored) {}
                try {
                    jfd.close();
                } catch (SqlJetException ignored) { }
                journalOpen = false;
                if (rc == null && !tempFile) {
                    try {
                        if (!fileSystem.delete(journal, true)) rc = new SqlJetIOException(SqlJetIOErrorCode.IOERR_DELETE);
                    } catch (SqlJetException e) {
                        rc = e;
                    }
                }
    ...
    private boolean hasHotJournal() throws SqlJetException {
        ...
        if (exists) locked = fd.checkReservedLock();
        if (exists && !locked) {
            if (0 == getPageCount()) {
                fileSystem.delete(journal, false);
            } else {
