...
public class SqlJetFileSystem implements ISqlJetFileSystem {
     ...
     * java.util.Set)
     */
    public ISqlJetFile open(final File path, final SqlJetFileType type, final Set<SqlJetFileOpenPermission> permissions) throws SqlJetException {

        if (null == type) throw new SqlJetException(SqlJetErrorCode.BAD_PARAMETER, "File type must not be null to open file");

        if (null == permissions || permissions.isEmpty()) throw new SqlJetException(SqlJetErrorCode.BAD_PARAMETER, "Permissions must not be null or empty to open file");
        ...
        if (!isReadWrite && !(filePath.isFile() && filePath.canRead())) throw new SqlJetException(SqlJetErrorCode.CANTOPEN);
    ...
    public byte[] randomness(int numBytes) {
        assert (numBytes > 0) : "numBytes must be greater than 0";
    ...
    public long sleep(long microseconds) {
        assert (microseconds > 0) : "microseconds must be greater than 0";
