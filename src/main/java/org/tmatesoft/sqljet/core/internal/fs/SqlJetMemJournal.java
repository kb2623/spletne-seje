...
public class SqlJetMemJournal implements ISqlJetFile {
    ...
    public int read(ISqlJetMemoryPointer buffer, int amount, long offset) {
        ...
        assert (offset + amount <= p.endpoint.iOffset);

        if (p.readpoint.iOffset != offset || offset == 0) {
            long iOff = 0;
            for (pChunk = p.pFirst; pChunk != null && (iOff + JOURNAL_CHUNKSIZE) <= offset; pChunk = pChunk.pNext) iOff += JOURNAL_CHUNKSIZE;
        } else {
        ...
        p.readpoint.iOffset = offset + amount;
    ...
    public void write(ISqlJetMemoryPointer buffer, int amount, long offset) {
         ...
         */
        assert (offset == p.endpoint.iOffset);
