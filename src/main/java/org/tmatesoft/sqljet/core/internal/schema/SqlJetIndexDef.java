...
public class SqlJetIndexDef extends SqlJetBaseIndexDef {
    ...
    SqlJetIndexDef(final String name, final String tableName, final int page, final String databaseName, final boolean unique, final boolean ifNotExists, final List<ISqlJetIndexedColumn> columns) {
    ...
    public String toString() {
        return String.valueOf(getPage()) + "/" + getRowId() + ": " + toSQL();
    }
    ...
    public String toSQL(boolean schemaStrict) {
        ...
        if (isUnique()) buffer.append("UNIQUE ");
        buffer.append("INDEX ");
        if (!schemaStrict) {
            if (isKeepExisting()) buffer.append("IF NOT EXISTS ");
            if (getDatabaseName() != null) buffer.append(getDatabaseName()).append('.');
        }
        buffer.append(getName()).append(" ON ").append(getTableName()).append(" (");
    ...
    public void bindColumns(ISqlJetTableDef tableDef) {
        for (final ISqlJetColumnDef tableColumn : tableDef.getColumns()) {
            columns.stream().filter(column -> column.getName().equalsIgnoreCase(tableColumn.getName())).forEach(column -> ((SqlJetIndexedColumn) column).setTableColumn(tableColumn));
        }
    }
