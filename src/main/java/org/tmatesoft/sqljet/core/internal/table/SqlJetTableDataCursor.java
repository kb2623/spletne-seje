...
public class SqlJetTableDataCursor extends SqlJetRowNumCursor {
    ...
    public long getRowId() throws SqlJetException {
        return (Long) db.runReadTransaction(db1 -> {
            final ISqlJetBtreeDataTable table = getBtreeDataTable();
            if (table.eof()) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Table is empty or the current record doesn't point to a data row");
    ...
    private int getFieldSafe(String fieldName) throws SqlJetException {
        final ISqlJetBtreeDataTable table = getBtreeDataTable();
        if (eof()) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Table is empty or the current record doesn't point to a data row");
        if (fieldName == null) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Field name is null");
        final int field = table.getDefinition().getColumnNumber(fieldName);
        if (field < 0) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Field not found: " + fieldName);
        return field;
    }

    public SqlJetValueType getFieldType(final String fieldName) throws SqlJetException {
        return (SqlJetValueType) db.runReadTransaction(db1 -> getBtreeDataTable().getFieldType(getFieldSafe(fieldName)));
    }

    public boolean isNull(final String fieldName) throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> getBtreeDataTable().isNull(getFieldSafe(fieldName)));
    }

    public String getString(final String fieldName) throws SqlJetException {
        return (String) db.runReadTransaction(db1 -> getBtreeDataTable().getString(getFieldSafe(fieldName)));
    }

    public long getInteger(final String fieldName) throws SqlJetException {
        return (Long) db.runReadTransaction(db1 -> SqlJetBtreeDataTable.isFieldNameRowId(fieldName) ? getBtreeDataTable().getRowId() : getBtreeDataTable().getInteger(getFieldSafe(fieldName)));
    }

    public double getFloat(final String fieldName) throws SqlJetException {
        return (Double) db.runReadTransaction(db1 -> getBtreeDataTable().getFloat(getFieldSafe(fieldName)));
    }

    public byte[] getBlobAsArray(final String fieldName) throws SqlJetException {
        return (byte[]) db.runReadTransaction(db1 -> {
    ...
    public InputStream getBlobAsStream(final String fieldName) throws SqlJetException {
        return (InputStream) db.runReadTransaction(db1 -> {
            ISqlJetMemoryPointer buffer = getBtreeDataTable().getBlob(getFieldSafe(fieldName));
    ...
    public Object getValue(final String fieldName) throws SqlJetException {
        return db.runReadTransaction(db1 -> SqlJetBtreeDataTable.isFieldNameRowId(fieldName) ? getBtreeDataTable().getRowId() : getBtreeDataTable().getValue(getFieldSafe(fieldName)));
    }

    public boolean getBoolean(final String fieldName) throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> getBoolean(getFieldSafe(fieldName)));
    }
    ...
    public void updateOr(final SqlJetConflictAction onConflict, final Object... values) throws SqlJetException {
        db.runWriteTransaction(db1 -> {
            final ISqlJetBtreeDataTable table = getBtreeDataTable();
            if (table.eof()) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Table is empty or current record doesn't't point to data row");
    ...
    public long updateWithRowIdOr(final SqlJetConflictAction onConflict, final long rowId, final Object... values) throws SqlJetException {
        return (Long) db.runWriteTransaction(db1 -> {
            final ISqlJetBtreeDataTable table = getBtreeDataTable();
            if (table.eof()) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Table is empty or current record doesn't't point to data row");
    ...
    public void updateByFieldNamesOr(final SqlJetConflictAction onConflict, final Map<String, Object> values)
        ...
        db.runWriteTransaction(db1 -> {
            final ISqlJetBtreeDataTable table = getBtreeDataTable();
            if (table.eof()) throw new SqlJetException(SqlJetErrorCode.MISUSE, "Table is empty or current record doesn't point to data row");
    ...
    public Object[] getRowValues() throws SqlJetException {
        return (Object[]) db.runReadTransaction(db1 -> getBtreeDataTable().getValues().clone());
    }
