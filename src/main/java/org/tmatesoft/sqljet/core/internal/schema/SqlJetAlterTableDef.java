...
public class SqlJetAlterTableDef {
    ...
    public SqlJetAlterTableDef(ParserRuleReturnScope parsedSql) throws SqlJetException {
        ...
        if (childCount < 5) throw new SqlJetException(SqlJetErrorCode.MISUSE, INVALID_ALTER_TABLE_STATEMENT);
        final CommonTree alterNode = (CommonTree) ast.getChild(0);
        final CommonTree tableNode = (CommonTree) ast.getChild(1);
        if (!"alter".equalsIgnoreCase(alterNode.getText()) || !"table".equalsIgnoreCase(tableNode.getText())) throw new SqlJetException(SqlJetErrorCode.MISUSE, INVALID_ALTER_TABLE_STATEMENT);
        ...
        if ("add".equalsIgnoreCase(action)) {
            ...
            if ("column".equalsIgnoreCase(child.getText())) {
                if (childCount != 6) throw new SqlJetException(SqlJetErrorCode.MISUSE, INVALID_ALTER_TABLE_STATEMENT);
