...
public class SqlJetVdbeSerialType {
    ...
    public static int serialTypeLen(int serial_type) {
        return serial_type >= 12 ? (serial_type - 12) / 2 : aSize[serial_type];
    }
    ...
    public static int serialType(SqlJetVdbeMem pMem, int file_format) {
        if (pMem.flags.contains(SqlJetVdbeMemFlags.Null)) return 0;
        int n;
        if (pMem.flags.contains(SqlJetVdbeMemFlags.Int)) {
            ...
            long i = pMem.i;
            long u;
            if (file_format >= 4 && (i & 1) == i) return 8 + (int) i;

            u = SqlJetUtility.absolute(i);

            if (u <= 127)            return 1;
            else if (u <= 32767)     return 2;
            else if (u <= 8388607)   return 3;
            else if (u <= 2147483647)return 4;
            else if (u <= MAX_6BYTE) return 5;
            else                     return 6;
        }
        if (pMem.flags.contains(SqlJetVdbeMemFlags.Real)) return 7;
        n = pMem.n;
        if (pMem.flags.contains(SqlJetVdbeMemFlags.Zero)) n += pMem.nZero;
        assert (n >= 0);
