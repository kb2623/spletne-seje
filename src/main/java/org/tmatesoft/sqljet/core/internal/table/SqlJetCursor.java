...
public abstract class SqlJetCursor implements ISqlJetCursor {
    ...
    public boolean eof() throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.eof());
    }

    public boolean first() throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.first());
    }

    public boolean last() throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.last());
    }

    public boolean next() throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.next());
    }

    public boolean previous() throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.previous());
    }

    public int getFieldsCount() throws SqlJetException {
        return (Integer) db.runReadTransaction(db1 -> btreeTable.getFieldsCount());
    }

    public SqlJetValueType getFieldType(final int field) throws SqlJetException {
        return (SqlJetValueType) db.runReadTransaction(db1 -> btreeTable.getFieldType(field));
    }

    public boolean isNull(final int field) throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.isNull(field));
    }

    public String getString(final int field) throws SqlJetException {
        return (String) db.runReadTransaction(db1 -> btreeTable.getString(field));
    }

    public long getInteger(final int field) throws SqlJetException {
        return (Long) db.runReadTransaction(db1 -> btreeTable.getInteger(field));
    }

    public double getFloat(final int field) throws SqlJetException {
        return (Double) db.runReadTransaction(db1 -> btreeTable.getFloat(field));
    }
    ...
    public Object getValue(final int field) throws SqlJetException {
        return db.runReadTransaction(db1 -> {
    ...
    public boolean getBoolean(final int field) throws SqlJetException {
        return (Boolean) db.runReadTransaction(db1 -> btreeTable.getInteger(field) != 0);
    }
