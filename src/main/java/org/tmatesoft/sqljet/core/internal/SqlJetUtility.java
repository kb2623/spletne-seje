...
public final class SqlJetUtility {
    ...
    public static void log(Logger logger, String format, Object... args) {
        ...
        if (SQLJET_LOG_STACKTRACE) logStackTrace(s);
    ...
    public static <T extends Enum<T>> T getEnumSysProp(String propName, T defValue) {
        if (null == propName) throw new SqlJetError("Undefined property name");
        if (null == defValue) throw new SqlJetError("Undefined default value");
        try {
    ...
    public static boolean bitSetTest(BitSet bitSet, int index) {
        ...
        return index >= 0 && bitSet.get(index);
    }
    ...
    public static int strlen30(ISqlJetMemoryPointer z) {
        int i = 0;
        for (final int l = z.getPointer(); i < l && SqlJetUtility.getUnsignedByte(z, i) != 0; i++);
        return 0x3fffffff & i;
    }
