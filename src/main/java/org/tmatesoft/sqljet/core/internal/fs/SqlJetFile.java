...
public class SqlJetFile implements ISqlJetFile {
    ...
    private static final boolean SQLJET_LOG_FILES_PERFORMANCE = SqlJetUtility.getBoolSysProp( SqlJetLogDefinitions.SQLJET_LOG_FILES_PERFORMANCE, false);
    ...
    private static void OSTRACE(String format, Object... args) {
        if (SQLJET_LOG_FILES) SqlJetUtility.log(filesLogger, format, args);
    }
    ...
    private void TIMER_START() {
        if (SQLJET_LOG_FILES_PERFORMANCE) timer_start = System.nanoTime();
    }
    ...
    public synchronized void close() throws SqlJetException {
        ...
        if (filePath != null && permissions.contains(SqlJetFileOpenPermission.DELETEONCLOSE)) {
            if (!SqlJetFileUtil.deleteFile(filePath)) throw new SqlJetIOException(SqlJetIOErrorCode.IOERR_DELETE, String.format("Can't delete file '%s'", filePath.getPath()));
        }
    ...
    public synchronized boolean lock(final SqlJetLockType lockType) throws SqlJetIOException {
        ...
        try {
            synchronized (openFiles) {
                 ...
                 * BUSY.
                 */
                if (this.lockType != lockInfo.lockType && (SqlJetLockType.PENDING.compareTo(lockInfo.lockType) <= 0 || SqlJetLockType.SHARED.compareTo(lockType) < 0))
                    return false;
                 ...
                 */
                if (lockType == SqlJetLockType.SHARED && lockInfo.sharedLockCount > 0 && (lockInfo.lockType == SqlJetLockType.SHARED || lockInfo.lockType == SqlJetLockType.RESERVED)) {
                ...
                if (lockType == SqlJetLockType.SHARED || (lockType == SqlJetLockType.EXCLUSIVE && this.lockType.compareTo(SqlJetLockType.PENDING) < 0)) {
                    ...
                    if (!locks.containsKey(SqlJetLockType.PENDING)) {
                        final FileLock pendingLock = fileLockManager.tryLock(PENDING_BYTE, 1, lockType == SqlJetLockType.SHARED);
