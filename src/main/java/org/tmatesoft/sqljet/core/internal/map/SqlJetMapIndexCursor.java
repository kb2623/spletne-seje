...
public class SqlJetMapIndexCursor extends SqlJetBtreeTable implements ISqlJetMapIndexCursor {
     ...
     */
    public SqlJetMapIndexCursor(final SqlJetMapDb mapDb, ISqlJetBtree btree, ISqlJetIndexDef indexDef, boolean writable) throws SqlJetException {
    ...
    public boolean goToKey(Object[] key) throws SqlJetException {
        if (key != null && key.length > 0) {
            ...
            if (moveTo < 0 && !next()) return false;
            if (moveTo != 0) {
                final ISqlJetBtreeRecord record = getRecord();
                if (null == record) return false;
                if (keyCompare(key.length, pKey, record.getRawRecord()) != 0) return false;
            }
            return true;
        } else {
    ...
    public void put(Object[] key, Long value) throws SqlJetException {
        if (write) {
            if (value != null) {
                ...
                try {
                    final SqlJetEncoding encoding = mapDb.getOptions().getEncoding();
                    final ISqlJetBtreeRecord rec = SqlJetBtreeRecord.getRecord(encoding, SqlJetUtility.addArrays(key, new Object[] { value }));
