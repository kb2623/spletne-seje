...
public class SqlJetMapTableCursor extends SqlJetBtreeTable implements ISqlJetMapTableCursor {
     ...
     */
    public SqlJetMapTableCursor(final SqlJetMapDb mapDb, ISqlJetBtree btree, SqlJetMapDef mapDef, boolean writable) throws SqlJetException {
    ...
    public boolean goToKey(long key) throws SqlJetException {
        if (key > 0) {
            ...
            if (moveTo != 0) {
                if (key != getKeySize()) return false;
            }
            return true;
        } else {
