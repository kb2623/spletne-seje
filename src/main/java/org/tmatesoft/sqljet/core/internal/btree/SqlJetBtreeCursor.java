...
public class SqlJetBtreeCursor extends SqlJetCloneable implements ISqlJetBtreeCursor {
    ...
    public SqlJetBtreeCursor(SqlJetBtree btree, int table, boolean wrFlag, ISqlJetKeyInfo keyInfo) throws SqlJetException {
        ...
        if (wrFlag) {
            assert (!pBt.readOnly);
            if (pBt.readOnly) throw new SqlJetException(SqlJetErrorCode.READONLY);
            if (btree.checkReadLocks(table, null, 0)) throw new SqlJetException(SqlJetErrorCode.LOCKED);
        }

        if (pBt.pPage1 == null) btree.lockWithRetry();
        this.pgnoRoot = table;
    ...
    public int moveToUnpacked(ISqlJetUnpackedRecord pIdxKey, long intKey, boolean biasRight) throws SqlJetException {
        ...
        for (;;) {
            ...
            lwr = 0;
            upr = pPage.nCell - 1;
            if ((!pPage.intKey && pIdxKey == null) || upr < 0) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
            if (biasRight) {
    ...
    public boolean cursorHasMoved() {
        ...
        return (eState != CursorState.VALID || skip != 0);
    }
...
//    	  SqlJetException rc;
     ...
     * @throws SqlJetException
     */
    private void balance() throws SqlJetException {
        SqlJetBtreeCursor pCur = this;
    ...
    private void balance_nonroot(SqlJetMemPage pParent, int iParentIdx, ISqlJetMemoryPointer aOvflSpace, boolean isRoot) throws SqlJetException {
        ...
        if (aOvflSpace == null) throw new SqlJetException(SqlJetErrorCode.NOMEM);
        ...
        try {
		  ...
		  */
            ...
            for(i=0; i<k; i++){
		      ...
		      /* Set the pointer-map entry for the new sibling page. */
                    if (pBt.autoVacuum) pBt.ptrmapPut(pNew.pgno, SqlJetBtreeShared.PTRMAP_BTREE, pParent.pgno);
                }
            }
		  ...
		  */
		    ...
		    ** image.  */
                assert( nNew==1 );
                assert( apNew[0].nFree == (get2byte(apNew[0].aData.getMoved(5))-apNew[0].cellOffset-apNew[0].nCell*2));
    ...
    private SqlJetMemPage balance_deeper(SqlJetMemPage pRoot) throws SqlJetException {
        ...
        memcpy(pChild.aOvfl, pRoot.aOvfl);
    ...
    void releaseTempCursor() throws SqlJetException {
        ...
        for (int i = 0; i <= pCur.iPage; i++) pCur.apPage[i].pDbPage.unref();
        pCur.pKey = null;
    }
    ...
    public void insert(ISqlJetMemoryPointer pKey, long nKey, ISqlJetMemoryPointer pData, int nData, int zero, boolean bias) throws SqlJetException {
        ...
        if (pCur.eState == CursorState.FAULT) throw new SqlJetException(pCur.error);
    ...
    public boolean previous() throws SqlJetException {
        ...
        pCur.restoreCursorPosition();
        pCur.atLast = false;
        if (CursorState.INVALID == pCur.eState) return true;
        if (pCur.skip < 0) {
            pCur.skip = 0;
            return false;
        }
        ...
        if (!pPage.leaf) {
            ...
            pPage = pCur.apPage[pCur.iPage];
            if (pPage.intKey && !pPage.leaf) return pCur.previous();
        }
        return false;
    }
    ...
    public void key(int offset, int amt, ISqlJetMemoryPointer buf) throws SqlJetException {
        ...
        if (pCur.apPage[0].intKey) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
    ...
    private void accessPayload(int offset, int amt, ISqlJetMemoryPointer pBuf, int skipKey, boolean eOp)
        ...
        if (amt > 0) throw new SqlJetException(SqlJetErrorCode.CORRUPT);
    ...
    public ISqlJetMemoryPointer keyFetch(int[] amt) {
        ...
        return eState == CursorState.VALID ? fetchPayload(amt, false) : null;
    }
    ...
    public ISqlJetMemoryPointer dataFetch(int[] amt) {
        assert (cursorHoldsMutex(this));
        return eState == CursorState.VALID ? fetchPayload(amt, true) : null;
    }
    ...
    public void data(int offset, int amt, ISqlJetMemoryPointer buf) throws SqlJetException {
        ...
        if (eState == CursorState.INVALID) throw new SqlJetException(SqlJetErrorCode.ABORT);
    ...
    public void putData(int offset, int amt, ISqlJetMemoryPointer data) throws SqlJetException {
        ...
        if (this.eState != CursorState.VALID) throw new SqlJetException(SqlJetErrorCode.ABORT);
         ...
         */
        if (!this.wrFlag) throw new SqlJetException(SqlJetErrorCode.READONLY);
        assert (!this.pBt.readOnly && this.pBt.inTransaction == TransMode.WRITE);
        if (this.pBtree.checkReadLocks(this.pgnoRoot, this, 0))
            /* The table pCur points to has a read lock */
            throw new SqlJetException(SqlJetErrorCode.LOCKED);
        if (this.eState == CursorState.INVALID || !this.apPage[this.iPage].intKey) throw new SqlJetException(SqlJetErrorCode.ERROR);
