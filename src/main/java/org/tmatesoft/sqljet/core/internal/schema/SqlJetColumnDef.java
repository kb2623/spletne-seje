...
public class SqlJetColumnDef implements ISqlJetColumnDef {
    ...
    public boolean hasExactlyIntegerType() {
        if (getTypeAffinity() != SqlJetTypeAffinity.INTEGER) return false;
    ...
    public String toString() {
        ...
        if (getType() != null) buffer.append(' ').append(getType());
        for (int i = 0; i < getConstraints().size(); i++) buffer.append(' ').append(getConstraints().get(i));
