...
public class SqlJetEngine implements AutoCloseable {
	...
	public int getCacheSize() throws SqlJetException {
		checkOpen();
		refreshSchema();
		return (Integer) runSynchronized(engine -> btree.getCacheSize());
	}
    ...
    public SqlJetSafetyLevel getSafetyLevel() throws SqlJetException {
        checkOpen();
        refreshSchema();
        return (SqlJetSafetyLevel) runSynchronized(engine -> btree.getSafetyLevel());
    }
    ...
    public SqlJetPagerJournalMode getJournalMode() throws SqlJetException {
        checkOpen();
        refreshSchema();
        return (SqlJetPagerJournalMode) runSynchronized(engine -> btree.getJournalMode());
    }
