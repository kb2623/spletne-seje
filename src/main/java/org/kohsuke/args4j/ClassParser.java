    ...
    public void parse(Object bean, CmdLineParser parser) {
        ...
        for( Class c=bean.getClass(); c!=null; c=c.getSuperclass()) {
            for( Method m : c.getDeclaredMethods() ) {
                ...
                if (o != null) parser.addOption(new MethodSetter(parser,bean,m), o);
                Argument a = m.getAnnotation(Argument.class);
                if (a != null) parser.addArgument(new MethodSetter(parser,bean,m), a);
            }
            for( Field f : c.getDeclaredFields() ) {
                Option o = f.getAnnotation(Option.class);
                if (o != null) parser.addOption(Setters.create(f,bean),o);
                Argument a = f.getAnnotation(Argument.class);
                if (a != null) parser.addArgument(Setters.create(f,bean), a);
            }
        }
	}
}
