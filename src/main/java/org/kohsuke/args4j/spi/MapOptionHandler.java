...
public class MapOptionHandler extends OptionHandler<Map<?,?>> {
    ...
    public MapOptionHandler(CmdLineParser parser, OptionDef option, Setter<? super Map<?,?>> setter) {
        ...
        if (setter.asFieldSetter()==null) throw new IllegalArgumentException("MapOptionHandler can only work with fields");
    }
    ...
    protected void addToMap(String argument, Map m) throws CmdLineException {
        if (String.valueOf(argument).indexOf('=') == -1) throw new CmdLineException(owner,Messages.FORMAT_ERROR_FOR_MAP);
        ...
        if (mapKey.length()==0) throw new CmdLineException(owner,Messages.MAP_HAS_NO_KEY);
